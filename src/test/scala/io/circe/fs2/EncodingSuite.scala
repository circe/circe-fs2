...
class EncodingSuite extends AnyFlatSpec with Matchers {
  ...
  it should "encode a case class containing a stream" in {
    val streamed = Streamed[Pure, Simple](1, Stream(Simple("a"), Simple("b"), Simple("c")), Simple("2"))
    parse(streamed.asJsonStream[Pure].compile.string).flatMap(_.as[Listed[Simple]]) shouldBe Right(
      Listed.fromStreamed(streamed)
    )
  }
